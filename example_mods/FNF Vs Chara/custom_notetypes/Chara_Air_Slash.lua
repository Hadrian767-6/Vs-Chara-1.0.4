function onCreate()
	...
	for i = 0, getProperty('unspawnNotes.length')-1 do
		...
		if getPropertyFromGroup('unspawnNotes', i, 'noteType') == 'Chara_Air_Slash' then
...
attackAnimations = {'airAttackLEFT', 'airAttackDOWN', 'airAttackUP', 'airAttackRIGHT'}
function opponentNoteHit(id, noteData, noteType, isSustainNote)
	if noteType == 'Chara_Air_Slash' then

		characterPlayAnim('dad', ''..attackAnimations[noteData+1], true);
		setProperty('dad.specialAnim', true);
        if not isSustainNote then
            if attackAnimations[noteData+1] == 'airAttackLEFT' then
                setProperty('RightSlashChara.visible', true)
                doTweenX('Slashs', 'RightSlashChara', '1100','0.2', 'linear')
            end
            if attackAnimations[noteData+1] == 'airAttackUP' then
                setProperty('RightSlashChara.visible', true)
                doTweenX('Slashs', 'RightSlashChara', '1100','0.2', 'linear')
            end 
            if attackAnimations[noteData+1] == 'airAttackDOWN' then
                doTweenX('Slashs', 'RightSlashChara', '1100','0.2', 'linear')
                setProperty('RightSlashChara.visible', true)
            end
            if attackAnimations[noteData+1] == 'airAttackRIGHT' then
