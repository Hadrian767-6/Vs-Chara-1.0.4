function onCreate()
	...
	for i = 0, getProperty('unspawnNotes.length')-1 do
		...
		if getPropertyFromGroup('unspawnNotes', i, 'noteType') == 'alt_Chara_Air_Slash' then
...
function opponentNoteHit(id, noteData, noteType)
	if noteType == 'alt_Chara_Air_Slash' then

		characterPlayAnim('dad', ''..attackAnimations[noteData+1], true);
		setProperty('dad.specialAnim', true);

        if not isSustainNote then
            if attackAnimations[noteData+1] == 'alt-airAttackLEFT' then
                setProperty('ARightSlashChara.visible', true)
                doTweenX('Slashs', 'ARightSlashChara', '1100','0.2', 'linear')
            end
            if attackAnimations[noteData+1] == 'alt-airAttackUP' then
                setProperty('ARightSlashChara.visible', true)
                doTweenX('Slashs', 'ARightSlashChara', '1100','0.2', 'linear')
            end 
            if attackAnimations[noteData+1] == 'alt-airAttackDOWN' then
                doTweenX('Slashs', 'ARightSlashChara', '1100','0.2', 'linear')
                setProperty('ARightSlashChara.visible', true)
            end
            if attackAnimations[noteData+1] == 'alt-airAttackRIGHT' then
